...
if (mouse_check_button_pressed(mb_left)) {
	...
	if(objetos[ranura][3] == 1){
		...
		if(ranura >= 0 && objetos[ranura][0] != 0){
			...
			var nombre_objeto = objetos[ranura][5];		// Nombre del objeto
		...
		if (ranura >= 0 && objetos[ranura][0] != 0) { // Si hay un objeto en la ranura clickeada
			...
			if (tipo_objeto == "corta" && objetos[0][0] == 0) {
				...
				obj_personaje.equipo_corta = objetos[0][5];
			    ...
			    show_debug_message("Herramienta equipada en la ranura 4.");
			}
		}
	}
}
...
// Elimina objeto del inventario y del equipamiento
if (mouse_check_button_pressed(mb_right)) {
	...
	if (ranura >= 0 && objetos[ranura][0] != 0) {
		...
		if (cantidad_objeto >= 1) {
		    objetos[ranura][1] -= 1; // Reducir la cantidad
			
			if (objetos[ranura][1] == 0){
			...
			obj_personaje.equipo_corta = objetos[ranura][5];
		} else if (ranura == 1) {
		    // Vaciar la ranura original
			objetos[ranura][0] = 0;
			objetos[ranura][1] = 0;
			...
			obj_personaje.equipo_larga = objetos[ranura][5];
		} else if (ranura == 2) {
		    // Vaciar la ranura original
			objetos[ranura][0] = 0;
			objetos[ranura][1] = 0;
			...
			obj_personaje.equipo_escudo = objetos[ranura][5];
		} else if (ranura == 3) {
		    // Vaciar la ranura original
			objetos[ranura][0] = 0;
			objetos[ranura][1] = 0;
			...
			obj_personaje.equipo_herramienta = objetos[ranura][5];
		}
...
if (keyboard_check_pressed(ord("1"))) {
	show_debug_message(obj_personaje.equipo_corta);
